(function() {
  ...
  genImportExportConfig = function(ractive, viewController, compiler) {
    return {
      ...
      getNlogo: function() {
        ...
        ({result, success} = compiler.exportNlogo({
  ...
  genIOConfig = function(ractive) {
    return {
      ...
      slurpURLAsync: function(url) {
        return function(callback, reportErrors) {
          var reportError;
          reportError = function(ex) {
            // coffeelint: disable=max_line_length
            return reportErrors(["Extension exception: Could not fetch resource from the given URL. Make sure the URL is correct, that there are no network issues, and that CORS access is permitted. The exact error message is below.", "", ex.message]);
          };
          // coffeelint: enable=max_line_length
          fetch(url).then(function(response) {
            var serverError, serverMessage;
            if (!response.ok) {
              ...
              return reportErrors([serverError, "", serverMessage]);
              ...
              return response.text().then(callback).catch(reportError);
            }
          }).catch(reportError);
        };
      }
  ...
  window.genConfigs = function(ractive, viewController, container, compiler) {
    var appendToConsole;
